...
async function run() {
  ...
  // --- a history of completed swaps so trust scores are non-zero ---
  const history = [
  ...
  let created = 0;
  for (const [requesterEmail, providerEmail, skillRequested, skillOffered, type, hours] of history) {
    ...
    // Settle the credit swap through the same ledger the API uses.
    if (type === 'credit') {
    ...
    // Two reviews per completed swap; the Review post-save hook updates trust.
    await Review.create({
      swap: swap._id,
  ...
  // --- an open request so the inbox is not empty ---
  const pendingExists = await SwapRequest.findOne({ status: 'pending' });
  if (!pendingExists) {
    const swap = await SwapRequest.create({
  ...
  // --- recompute every trust score from the seeded history ---
  for (const user of users) {
    await recalculateTrustScore(user._id);
  }

  // Settle the consequences of that history: swaps written directly into the
  // database never went through the API, so nothing has evaluated the
  // challenges they satisfy. Without this a seeded member can sit at 100%
  // progress and never be paid.
  ...
  for (const u of summary) {
    logger.info(`  ${u.name.padEnd(16)} ${u.email.padEnd(28)} trust ${String(u.trustScore).padStart(3)}  ${u.wallet.balance} credits`);
  }
